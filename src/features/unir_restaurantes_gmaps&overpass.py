...
# Cargar el mapa base de NYC
coord_nyc = gpd.read_file(path_coord_nyc)
coord_nyc = coord_nyc.to_crs(epsg=4326)  # Asegúrate de que esté en el sistema de coordenadas correcto

# Crear GeoDataFrame con los datos de restaurantes
def crear_gdf(df):
    df = df.copy()
    df['geometry'] = df.apply(lambda row: Point(row['longitude'], row['latitude']), axis=1)
    return gpd.GeoDataFrame(df, geometry='geometry', crs='EPSG:4326')

# Crear un GeoDataFrame con todos los restaurantes (sin tener en cuenta la fuente)
gdf = crear_gdf(final_df)

# Normalizar las calificaciones para el color
min_rating = gdf['avg_rating'].min()
max_rating = gdf['avg_rating'].max()
...
# Dibujar los restaurantes con colores según la reseña
for _, row in gdf.iterrows():
    # Establecer color basado en la reseña
    color = cmap(norm(row['avg_rating']))
    ax.scatter(row['longitude'], row['latitude'], color=color, s=10, alpha=0.6)

# Agregar una barra de color
sm = plt.cm.ScalarMappable(cmap=cmap, norm=nor m)
sm.set_array([])
plt.colorbar(sm, ax=ax, label='Rating')

# Título y layout
plt.title("Mapa de calor de restaurantes en NYC (intensidad de reseña)", fontsize=14)
plt.tight_layout()
plt.show()